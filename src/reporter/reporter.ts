 ...
 * `configDir` is the only one this package relies on; the underscore-prefixed
 * pair is internal to Playwright and read defensively. */
...
export default class QualflareReporter implements Reporter {
  ...
  private collectCases(root: PwSuite, config: ResolvedReporterConfig): void {
    for (const test of root.allTests()) {
      const built = buildCase(test, config, this.attachmentsByResult);
      if (!built) {
        continue;
      }
  ...
  private writeReport(config: ResolvedReporterConfig): void {
    ...
    const outputDir = path.isAbsolute(config.outputDir)
      ? config.outputDir
      : path.resolve(this.options.configDir ?? this.rootDir, config.outputDir);
