...
1. Ensure `main` is green: `.github/workflows/ci.yml` passing (unit tests + the real-playwright
   integration suite across the full `{playwright, node}` version matrix).
...
5. Tag: `git tag vX.Y.Z && git push origin vX.Y.Z` — pushing the tag triggers
   ...
   publish pattern).
6. Confirm the publish succeeded: `npm view @qualflare/playwright version` should show the new
...
- [ ] The real-playwright integration suite (`npm run test:integration`) has been run successfully
      in CI across the full declared `peerDependencies["@playwright/test"]` range, not just locally.
